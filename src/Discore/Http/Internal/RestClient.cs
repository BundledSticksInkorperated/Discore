...
namespace Discore.Http.Internal
{
    class RestClient : IDisposable
    {
        ...
        async Task<JsonDocument?> ParseResponse(HttpResponseMessage response, RateLimitHeaders? rateLimitHeaders, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                catch (JsonException)
                {
                    ...
                    throw new DiscordHttpApiException("Failed to parse response.", DiscordHttpErrorCode.None, response.StatusCode);
                }
            }
